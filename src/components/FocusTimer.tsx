...
export const FocusTimer: React.FC<FocusTimerProps> = ({
  ...
  return (
    <div className="glass-panel-glow px-6 py-4 rounded-3xl flex flex-col items-center justify-between min-h-[175px] w-full max-w-md mx-auto shadow-2xl transition-all">
      {/* Top Header Row: Preset buttons (when idle) or Diving Depth Indicator (when diving) - Same Height */}
      <div className="h-8 flex items-center justify-center w-full">
        {!isDiving ? (
          <div className="flex items-center gap-2 p-1 bg-sky-950/60 rounded-2xl border border-sky-800/40">
            {PRESETS.map((p) => (
              <button
                key={p.minutes}
          ...
          <div className="flex items-center gap-2 px-4 py-1 bg-sky-950/80 rounded-2xl border border-sky-600/40 text-sky-300 text-xs font-semibold animate-pulse shadow-inner">
      ...
      <div className="relative flex flex-col items-center my-1">
        <div className="text-4xl font-bold tracking-wider font-mono text-white drop-shadow-[0_0_12px_rgba(56,189,248,0.5)]">
          {formatTime(timeLeft)}
        </div>

        {/* Progress bar (reserved space so height never changes) */}
        <div className="w-56 h-1.5 bg-sky-950/80 rounded-full mt-1.5 overflow-hidden border border-sky-700/40">
          <div
      ...
      <div className="flex items-center gap-3">
        <button
          ...
          className={`flex items-center gap-2 px-6 py-2.5 rounded-2xl font-bold text-sm shadow-lg transition-all transform active:scale-95 ${
            isDiving
